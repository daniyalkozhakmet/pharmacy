  ...
  return (
    <div>
      <div className="row align-items-center py-3">
        ...
        <NavDropdown title="Дәрі категориялары" id="basic-nav-dropdown">
        {category.length>0 && category.map(c=><NavDropdown.Item key={c._id} onClick={e=>searchHandler(c.name)}>{c.name}</NavDropdown.Item>)}
          </NavDropdown>
        </div>
        <div className="col-md-2 col-sm-6 ">
        <NavDropdown title="Дәрі өндірушілері" id="basic-nav-dropdown">
      ...
      <div className="row">
        {loading ? (
          ...
          <Row>
            {products.length == 0 ? (
              <Message variant="warning" children={"Сәйкестік табылмады"} />
            ) : (
